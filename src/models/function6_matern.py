...
class CakeRecipeModel:
    def __init__(
        ...
        ls_init, ls_bounds = self._ls_init_and_bounds_from_spacing(self._dataset[['x1', 'x2', 'x3', 'x4', 'x5']].to_numpy())
        self._kernel = C(0.5, (1e-3, 1e3)) * Matern(
    ...
    def get_correlation_matrix(self) -> np.ndarray:
        X = self._dataset[['x1', 'x2', 'x3', 'x4', 'x5']].to_numpy()
    ...
    def _suggest_length_scale_bounds(self) -> List[Tuple[float, float]]:
        ...
        base_gp = GaussianProcessRegressor(
            kernel=(C(0.5, (1e-3,1e3)) *
                Matern(length_scale=np.full(5, 0.15),
                       length_scale_bounds=[(1e-2, 1.0)]*5,
                       nu=1.5) +
                WhiteKernel(
                    noise_level=1e-3, 
                    noise_level_bounds=(1e-6, 1e1)
    ...
    def suggest_next_point_to_evaluate(self, bounds: List[Tuple[float, float]]) -> str: 
        self._model.fit(self._dataset[['x1', 'x2', 'x3', 'x4', 'x5']], self._dataset['y'])
