...
from domain.domain_models import WarpConfig
...
class HeteroskedasticModel(HeteroskedasticContract):
    def __init__(
        self, 
        ...
        if warp_outputs:
            self._y_warp = WarpedOutputTransformation(y=self._Y_train, lam_low=lam.lam_low, lam_high=lam.lam_high)
    ...
    def suggest_next_point_to_evaluate(
        ...
        return formatter(best_x), mu, std, max_acq


    def _maximize_acquisition_function(self, bounds: torch.Tensor, raw_samples: int, max_iter: int, acquisition_function) -> int:
        r = 0.2  # trust radius
        Y_warped = self._y_warp(self._Y_train) if self._warp_outputs else self._Y_train
        best_idx = torch.argmax(Y_warped)
        x_star = self._X_train[best_idx].detach()
        lb = (x_star - r).clamp(0.0, 1.0)
        ub = (x_star + r).clamp(0.0, 1.0)
        local_bounds = torch.stack([lb, ub])
