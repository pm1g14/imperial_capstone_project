...
class ChemicalProcessModel(HomoskedasticFewDModel):
    def __init__(
        self, 
    ...
    def suggest_next_point_to_evaluate(self, bounds: List[Tuple[float, float]]) -> str: 
        ...
        initial_mean = self._incumbent_mean(self._dataset[['x1', 'x2', 'x3', 'x4']].to_numpy())
    ...
    def _calculate_acquisition_function_y(self, x: np.ndarray, best_f: float | None = None) -> float:
        ...
        return float(self._acquisition_function(mu, sigma, best_f, xi=0.04))
    ...
    def _maximize_acquisition_function(self, bounds: List[Tuple[float, float]], best_f: float, method:str = "L-BFGS-B") -> int:
        best_x = None
        best_acq_y = -np.inf
