...
from src.utils.matern_bounds_calculator_utils import MaternBoundsCalculatorUtils
from tests.test_utils.warp_utils import WarpUtils
torch.manual_seed(30)
random.seed(30)
np.random.seed(30)

class HeteroskedasticModel:
    def __init__(
        self, 
        ...
        if warp_outputs:
            self._Y_train, _ = WarpUtils.warp_boxcox(self._Y_train)
    ...
    def _calculate_residuals(self, eps: float):
        ...
        resid = (self._Y_train - f_post.mean).squeeze(-1)
    ...
    def suggest_next_point_to_evaluate(
        ...
        acquisition_function: Union[qLogNoisyExpectedImprovement,UpperConfidenceBound,qNegIntegratedPosteriorVariance], 
...
class WrapperHeteroskedasticModel(Model):
    ...
    def posterior(
        self,
        X: torch.Tensor,
        output_indices: Union[list[int],None] = None,
        observation_noise: Union[bool,torch.Tensor] = True,
        posterior_transform: Union[PosteriorTransform, None] = None,
    ) -> Posterior:
        ...
        noise_var = noise_var.clamp(min=1e-4, max=1e4).squeeze(-1)

        
        mvn = f_post.mvn
        lazy_cov = mvn.lazy_covariance_matrix
        cov_obs_lazy = lazy_cov.add_diag(noise_var)
        cov_obs_lazy = cov_obs_lazy.add_jitter(1e-4)
        ...
        mvn_obs = gpytorch.distributions.MultivariateNormal(mvn.mean, cov_obs_lazy)
